...
def training_loop(
    ...
    disc_kwargs         = {},       # Options for discriminator.
    ...
    if disc_kwargs:
        ...
        disc = dnnlib.util.construct_class_by_name(**disc_kwargs, **interface_kwargs)
        disc.train().requires_grad_(True).to(device)
    
    if dist.get_rank() == 0:
        with torch.no_grad():
            images = torch.zeros([batch_gpu, net.img_channels, net.img_resolution, net.img_resolution], device=device)
            sigma = torch.ones([batch_gpu], device=device)
            ts = torch.ones([batch_gpu,2], device=device)
            labels = torch.zeros([batch_gpu, net.label_dim], device=device)
            misc.print_module_summary(net, [images, sigma, labels], max_nesting=2)
            if disc_kwargs:
                misc.print_module_summary(disc, [images, labels, ts], max_nesting=2)
    ...
    stats_jsonl = None
    img_size = net.img_channels * (net.img_resolution **2)
    while True:
