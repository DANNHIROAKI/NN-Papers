...
def train_worker(network, loader, optimizer, loss_function, size, p_id, p_outputs, batch_size):
    train_loss = []
    predictions = []
    ground_truth = []
    ...
    if p_id == 0:
        ...
        generator = tqdm(loader.generate(size=size, training=True), total=size)
    else:
        generator = loader.generate(size=size, training=True)
    ...
    for it, graph in enumerate(generator):
        ...
        gemme_features = graph['gemme_features']
        y = graph['y']
        adj_res = graph['adj_res']

        pred = network(one_hot, features, gemme_features, adj_res).squeeze()
...
def predict_single_graph(graph, network):
    ...
    gemme_features = graph[names.GEMME_FEATURES_FIELD]
    adj_res = graph[names.ADJ_RES_FIELD]
    local_predictions = network(one_hot, features, gemme_features, adj_res).squeeze().cpu().detach().numpy()
