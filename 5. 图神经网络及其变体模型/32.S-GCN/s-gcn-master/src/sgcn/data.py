...
class DataLoader:
    ...
    def build_graph(self, model_path, target_path=None):
        # sequences
        model_sequence = self.load_sequence(utils.path([model_path, names.MODEL_FILE_NAME]))
        if target_path is not None:
            target_sequence = self.load_sequence(utils.path([target_path, names.TARGET_FILE_NAME]))
        else:
            target_sequence = model_sequence
        ...
        if self.gpu is not None:
            return {
                names.MODEL_SEQUENCE_FIELD: model_sequence,
                names.TARGET_SEQUENCE_FIELD: target_sequence,
    ...
    @staticmethod
    def load_sequence(pdb_path):
        biopdb = PandasPdb().read_pdb(pdb_path)
        df = biopdb.df['ATOM']
        return (
                df.chain_id.astype(str) + '_' + df.residue_number.astype(str) + '_' + df.residue_name
