...
def main():
    ...
    parser.add_argument('--encoder', action="store", type=int, required=False, default=None)
    parser.add_argument('--message_passing', action="store", type=int, required=False, default=None)
    parser.add_argument('--scorer', action="store", type=int, required=False, default=None)
    ...
    for checkpoint in utils.get_epochs(args):
        ...
        if utils.predictions_are_computed(checkpoint, prediction_output):
...
if __name__ == '__main__':
    ...
    from src.baseline import data, evaluation, networks, workers
    main()
