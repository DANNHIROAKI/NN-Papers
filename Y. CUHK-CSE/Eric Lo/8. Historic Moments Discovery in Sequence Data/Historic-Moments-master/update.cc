...
void Prune(RTree *rt, vector<sjvalue> N) //remove streaks in Rtree if dominated by skyline(Non-overlap)
{
	...
	cout<<GetFileSize(RTFILE2)<<endl;
	cout<<"Here delete # of entries: "<<count<<endl;
	return;
}
...
int main(int argc, char* argv[]) 
{
	...
	// below are for queries parameters
	...
	//below are for updating parameters
	part = 0.95; // pre-compute this part data
	const int num = 5; // 5 update portions
	float parts[num] = {0.001, 0.005, 0.01, 0.02, 0.05};
	//int num = 5; // the remaining dataset is divided into num sub-part, update 1 sub-part, 2 sub-parts ... num sub-parts
    ...
    {  	
        ...
        {    
            if(pdirent->d_name[0]='.')
				...
				{
					fout<<"SOIA\tMaintain\tLookup\tTotal\tSpace\tColumn"<<endl;
					sim_degree = sim_low; //initialization
            		...
            		// for update
            		for (int dt = 0; dt < num; dt++)
            		{
            			float part2 = parts[dt];
            			t0 = clock(); //begin to update the streaks
            			vector<lrvalue> BigDelta;
           				for (int number = int(Data.size()*part); number < int(Data.size()* (part+part2)); number++)
           				{
           					AdvanceUpdateOneData(rt2, Data[number],LPSK,BigDelta);
           				}
           				
           				vector<sjvalue> N;
           				vector<Oss> overprunerPair2 = RefreshOverpruner(overprunerPair, BigDelta, LPSK, N);
           				Prune(rt, N);
           				Rule(rt, overprunerPair);
           				t1 = clock(); 
          // 				cout<<"the size of N ???     ---> " <<N.size()<<endl;
	            		t2 = clock();
						...
						fout<<part2<<"\t"<<1000*double(t1-t0)/CLOCKS_PER_SEC<<"\t"<<1000*double(t3-t2)/(total_run_times * CLOCKS_PER_SEC)<<"\t"<<1000*double(t1-t0)/CLOCKS_PER_SEC + 1000*double(t3-t2)/(total_run_times * CLOCKS_PER_SEC)<<"\t"<<tree_size<<"\t"<<dt+1<<endl;	
            		}
            		fout.close();
				}
