...
#define PATH "/Users/lizbai/Documents/Datasets"
#define FILENAME "did"
    ...
    {  	
        ...
        {    
...

            {
            	...
            	strcat(tp, argv[1]);
            	ofstream fout(tp);
            	fout<<"sim_degree\tTime\tTime\tSpace"<<endl;
				...
				{
					cout<< " --- File "<<pdirent->d_name<<" ----- "<<endl;
					for (float flag = initial ; flag <= end; flag+=gap)
            		{
            			sim_low = flag;
            			sim_degree = sim_low;
            			t0 = clock();
						...
						//cout<<"check check --->  # of MaxStreaks: "<<MaxStreaks.size()<<endl;
						vector<Oss> overprunerPair = MakeOverPruner (LPSK, MaxStreaks);
	            		data_size = MaxStreaks.size();	
	            		//cout<<"check check --->  # of overpruner: "<<overprunerPair.size()<<endl;
	            		...
	            		for(int k = 0; k < total_run_times; k++) // five cases
						{
							...
							for (int j = 0; j < many; j++)
							{
								...
								//cout<<"**  Para:: "<<sim_degree<<" ,  "<<topk_option<<endl;
						...
						fout<<sim_low<<"\t"<<1000*double(t1-t0)/CLOCKS_PER_SEC<<"\t"<<1000*double(t3-t2)/(total_run_times * CLOCKS_PER_SEC)<<"\t"<<tree_size<<endl;	
            		}
            		fout.close();
				}
			}
